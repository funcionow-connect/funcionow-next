...
export default function CampanhasPage() {
  ...
  const filteredCampanhas = useMemo(() => campanhas.filter((item) => {
    const term = search.trim().toLowerCase();
    return (!term || item.nome.toLowerCase().includes(term) || (item.descricao || "").toLowerCase().includes(term)) && (statusFilter === "todas" || item.status === statusFilter);
  ...
  const handleCreate = async (event: FormEvent) => {
    ...
    const result = usuario?.empresa_id ? await supabase.from("campanhas").insert({ nome: nome.trim(), descricao: descricao.trim() || null, status, empresa_id: usuario.empresa_id, data_inicio: dataInicio || null, data_fim: dataFim || null, orcamento: Number(orcamento) || 0, receita: Number(receita) || 0 }) : { error: new Error("Usuário sem empresa") };
    setSaving(false); if (result.error) { setError(result.error.message); return; }
    setNome(""); setDescricao(""); setDataInicio(""); setDataFim(""); setOrcamento(""); setReceita(""); setShowForm(false); await loadCampanhas();
  };
  ...
  return <div>
    ...
    {showForm && <form onSubmit={handleCreate} style={formCard}><input required value={nome} onChange={(event) => setNome(event.target.value)} placeholder="Nome da campanha" style={input} /><input value={descricao} onChange={(event) => setDescricao(event.target.value)} placeholder="Descrição (opcional)" style={input} /><select value={status} onChange={(event) => setStatus(event.target.value as Status)} style={input}>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input type="date" value={dataInicio} onChange={(event) => setDataInicio(event.target.value)} style={input} /><input type="date" value={dataFim} onChange={(event) => setDataFim(event.target.value)} style={input} /><input type="number" min="0" value={orcamento} onChange={(event) => setOrcamento(event.target.value)} placeholder="Orçamento (R$)" style={input} /><input type="number" min="0" value={receita} onChange={(event) => setReceita(event.target.value)} placeholder="Receita prevista (R$)" style={input} /><button disabled={saving} style={primaryButton}>{saving ? "Salvando..." : "Salvar campanha"}</button></form>}
    ...
    <div style={filters}><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nome ou descrição" style={input} /><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as Status | "todas")} style={input}><option value="todas">Todos os status</option>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><span style={filterCount}>{filteredCampanhas.length} campanha(s)</span></div>
    {loading ? <div style={empty}>Carregando campanhas...</div> : campanhas.length === 0 ? <div style={empty}>Nenhuma campanha cadastrada ainda.</div> : filteredCampanhas.length === 0 ? <div style={empty}>Nenhuma campanha corresponde aos filtros.</div> : <div style={grid}>{filteredCampanhas.map((campanha) => <Link key={campanha.campanha_id} href={`/campanhas/detail?id=${campanha.campanha_id}`} style={card}><div style={cardTop}><strong>{campanha.nome}</strong><span style={statusStyle(campanha.status)}>{labels[campanha.status]}</span></div>{campanha.descricao && <p style={description}>{campanha.descricao}</p>}<div style={metrics}><Metric label="Creators" value={campanha.creators} /><Metric label="Entregáveis" value={`${campanha.concluidos}/${campanha.total}`} /><Metric label="ROI" value={campanha.orcamento ? `${((Number(campanha.receita || 0) / Number(campanha.orcamento)) || 0).toFixed(1)}x` : "-"} /></div><div style={footer}><span>Custo: {money(campanha.orcamento)}</span><span>Receita: {money(campanha.receita)}</span></div></Link>)}</div>}
  </div>;
}
...
const formCard = { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "8px", background: "white", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "12px", marginBottom: "14px" };
...
const filters = { display: "grid", gridTemplateColumns: "minmax(220px, 2fr) minmax(160px, 1fr) auto", gap: "8px", alignItems: "center", marginBottom: "14px" };
const filterCount = { color: "#6b7280", fontSize: "11px" };
...
const description = { color: "#6b7280", fontSize: "11px", lineHeight: 1.4, margin: "9px 0 0" };
